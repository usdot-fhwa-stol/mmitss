...
bool carma_v2x_msgs__msg__traffic_control_request_v01__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("carma_v2x_msgs.msg._traffic_control_request_v01.TrafficControlRequestV01", full_classname_dest, 72) == 0);
  }
  carma_v2x_msgs__msg__TrafficControlRequestV01 * ros_message = _ros_message;
  {  // reqid
  ...
  {  // bounds
    ...
    if (!carma_v2x_msgs__msg__TrafficControlBounds__Sequence__init(&(ros_message->bounds), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create carma_v2x_msgs__msg__TrafficControlBounds__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    carma_v2x_msgs__msg__TrafficControlBounds * dest = ros_message->bounds.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!carma_v2x_msgs__msg__traffic_control_bounds__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * carma_v2x_msgs__msg__traffic_control_request_v01__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("carma_v2x_msgs.msg._traffic_control_request_v01");
  ...
  {  // bounds
    ...
    carma_v2x_msgs__msg__TrafficControlBounds * item;
    for (size_t i = 0; i < size; ++i) {
      item = &(ros_message->bounds.data[i]);
      PyObject * pyitem = carma_v2x_msgs__msg__traffic_control_bounds__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
