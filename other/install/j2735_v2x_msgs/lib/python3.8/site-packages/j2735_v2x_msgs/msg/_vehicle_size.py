...
class Metaclass_VehicleSize(type):
    ...
    def __prepare__(cls, name, bases, **kwargs):
        ...
        return {
            'VEHICLE_WIDTH_UNAVAILABLE': cls.__constants['VEHICLE_WIDTH_UNAVAILABLE'],
            'VEHICLE_WIDTH_MAX': cls.__constants['VEHICLE_WIDTH_MAX'],
            'VEHICLE_WIDTH_MIN': cls.__constants['VEHICLE_WIDTH_MIN'],
            'VEHICLE_LENGTH_UNAVAILABLE': cls.__constants['VEHICLE_LENGTH_UNAVAILABLE'],
            'VEHICLE_LENGTH_MAX': cls.__constants['VEHICLE_LENGTH_MAX'],
            'VEHICLE_LENGTH_MIN': cls.__constants['VEHICLE_LENGTH_MIN'],
        }

    @property
    def VEHICLE_WIDTH_UNAVAILABLE(self):
        """Message constant 'VEHICLE_WIDTH_UNAVAILABLE'."""
        return Metaclass_VehicleSize.__constants['VEHICLE_WIDTH_UNAVAILABLE']
    ...
    @property
    def VEHICLE_LENGTH_UNAVAILABLE(self):
        """Message constant 'VEHICLE_LENGTH_UNAVAILABLE'."""
        return Metaclass_VehicleSize.__constants['VEHICLE_LENGTH_UNAVAILABLE']
...
class VehicleSize(metaclass=Metaclass_VehicleSize):
    ...
    def __init__(self, **kwargs):
        ...
        self.vehicle_width = kwargs.get('vehicle_width', int())
        self.vehicle_length = kwargs.get('vehicle_length', int())
    ...
    def vehicle_width(self, value):
        if __debug__:
            assert \
                ...
                "The 'vehicle_width' field must be of type 'int'"
            assert value >= 0 and value < 65536, \
                "The 'vehicle_width' field must be an unsigned integer in [0, 65535]"
    ...
    def vehicle_length(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'vehicle_length' field must be of type 'int'"
            assert value >= 0 and value < 65536, \
                "The 'vehicle_length' field must be an unsigned integer in [0, 65535]"
