  ...
  j2735_v2x_msgs__msg__AnimalType__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    j2735_v2x_msgs__msg__AnimalType * data =
      (j2735_v2x_msgs__msg__AnimalType *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!j2735_v2x_msgs__msg__AnimalType__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          j2735_v2x_msgs__msg__AnimalType__fini(&data[i]);
        }
        free(data);
        return false;
      }
