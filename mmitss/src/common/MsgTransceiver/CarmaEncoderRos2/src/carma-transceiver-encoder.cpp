...
std::vector<uint8_t> TransceiverEncoder::TransceiverEncoder::SRMEncoder(std::string jsonString)
{
...
std::vector<uint8_t> TransceiverEncoder::SPaTEncoder(std::string jsonString)
{
    Json::Value jsonObject;
	Json::CharReaderBuilder builder;
	Json::CharReader *reader = builder.newCharReader();
...
std::vector<uint8_t> TransceiverEncoder::SSMEncoder(std::string jsonString)
{
    ...
    std::vector<int> expectedTimeOfArrival_Second = signalStatus.getETA_Second();
    std::vector<int> expectedTimeOfArrival_Duration = signalStatus.getETA_Duration();
